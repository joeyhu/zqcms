...
import {
  LayoutDashboard, FileText, FolderTree, Settings, Image, Blocks, LogOut,
  Globe, ChevronDown, Plus, Wrench, FolderOpen, Layers,
} from 'lucide-react';
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="flex h-14 items-center justify-between border-b bg-white px-6">
          <span className="text-sm text-gray-500">
            {currentSite ? `当前站点: ${currentSite.name} (${currentSite.domain})` : '未选择站点'}
          </span>
              ...
              <div className="absolute right-0 top-full z-50 mt-1 w-64 rounded-xl border bg-white shadow-lg">
                <div className="max-h-60 overflow-y-auto py-2">
                  {sites.map((site) => (
                    ...
                    >
                      <span className={`h-2 w-2 rounded-full ${currentSite?.id === site.id ? 'bg-blue-600' : 'bg-gray-300'}`} />
                ...
                <div className="border-t px-4 py-2">
                  <button
                    onClick={() => { setMenuOpen(false); navigate('/sites/new'); }}
