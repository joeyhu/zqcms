...
export function CategoryFormPage() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      fetchAPI<Category>(`/categories/${id}`).then((cat) => {
        setName(cat.name);
        setSlug(cat.slug);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const body = { name, slug, description: description || null, parentId, isVisible, sortOrder };

    try {
      if (isEdit) {
        await fetchAPI(`/categories/${id}`, { method: 'PUT', body: JSON.stringify(body) });
        toast.success('目录已更新');
      } else {
        await fetchAPI('/categories', { method: 'POST', body: JSON.stringify(body) });
        toast.success('目录已创建');
      }
      navigate('/categories');
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{isEdit ? '编辑目录' : '新建目录'}</h1>
        <button onClick={() => navigate('/categories')} className="text-sm text-gray-500">返回列表</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">父目录</label>
