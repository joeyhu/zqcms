...
import {
  Plus, Trash2, GripVertical, Settings, Eye, EyeOff,
...
function BlockConfigPanel({
  ...
  block: PageBlock;
  onSave: (id: number, data: { config: Record<string, unknown>; title?: string }) => void;
  onClose: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto">
          ...
          {block.blockType === BlockType.HERO && (
            <>
              <Field label="标题" value={config.title as string} onChange={(v) => updateField('title', v)} />
              <Field label="副标题" value={config.subtitle as string} onChange={(v) => updateField('subtitle', v)} />
              <Field label="主按钮文字" value={config.ctaText as string} onChange={(v) => updateField('ctaText', v)} />
              <Field label="主按钮链接" value={config.ctaLink as string} onChange={(v) => updateField('ctaLink', v)} />
              <Field label="次按钮文字" value={config.secondaryCtaText as string} onChange={(v) => updateField('secondaryCtaText', v)} />
              <Field label="次按钮链接" value={config.secondaryCtaLink as string} onChange={(v) => updateField('secondaryCtaLink', v)} />
              <Field label="背景图片 URL" value={config.bgImage as string} onChange={(v) => updateField('bgImage', v)} />
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">对齐方式</label>
                <select value={(config.alignment as string) || 'center'}
          ...
          {block.blockType === BlockType.FEATURES && (
            <Field label="列数" value={String(config.columns || 3)} onChange={(v) => updateField('columns', Number(v))} />
          )}

          {block.blockType === BlockType.POST_LIST && (
            <>
              <Field label="显示数量" value={String(config.limit || 6)} onChange={(v) => updateField('limit', Number(v))} />
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">布局</label>
                <select value={(config.layout as string) || 'grid'}
          ...
          {block.blockType === BlockType.CATEGORY_LIST && (
            <>
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={config.showCount !== false}
                  onChange={(e) => updateField('showCount', e.target.checked)} className="rounded" />
                显示文章数量
              </label>
            </>
          )}
          ...
          {block.blockType === BlockType.FAQ && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">FAQ 数据 (JSON)</label>
              <textarea value={JSON.stringify(config.items || [], null, 2)}
                onChange={(e) => { try { updateField('items', JSON.parse(e.target.value)); } catch {} }}
                rows={6} className="block w-full rounded-lg border px-3 py-2 text-sm font-mono" />
            </div>
          )}
          ...
          {block.blockType === BlockType.MARKDOWN && (
            <div>
              ...
              <textarea value={(config.content as string) || ''}
                onChange={(e) => updateField('content', e.target.value)}
                rows={8} className="block w-full rounded-lg border px-3 py-2 text-sm font-mono" />
            </div>
          )}
          ...
          {block.blockType === BlockType.CTA && (
            <>
              <Field label="标题" value={config.title as string} onChange={(v) => updateField('title', v)} />
              <Field label="描述" value={config.desc as string} onChange={(v) => updateField('desc', v)} />
              <Field label="按钮文字" value={config.btnText as string} onChange={(v) => updateField('btnText', v)} />
              <Field label="按钮链接" value={config.btnLink as string} onChange={(v) => updateField('btnLink', v)} />
            </>
          )}

          {/* Raw JSON editor for all types */}
          <details>
            <summary className="text-sm text-gray-500 cursor-pointer">高级：JSON 编辑</summary>
            <textarea value={JSON.stringify(config, null, 2)}
              onChange={(e) => { try { setConfig(JSON.parse(e.target.value)); } catch {} }}
              rows={8} className="mt-2 block w-full rounded-lg border px-3 py-2 text-xs font-mono" />
          </details>
        </div>

        <div className="mt-6 flex gap-3 justify-end pt-4 border-t">
          <button onClick={onClose} className="rounded-lg border px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">取消</button>
          <button onClick={() => onSave(block.id, { config, title: title || undefined })}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {block.title && (
            <span className="text-xs text-gray-600 truncate">{block.title}</span>
          )}
        </div>
...
export function PageBuilderPage() {
  ...
  const { pageType, pageLabel, basePageType, slug } = useMemo(() => {
    ...
    const labels: Record<string, string> = {
      home: '首页搭建', category: '目录页模板搭建', subcategory: '子目录页模板搭建',
    };
    let label = labels[base] || '页面搭建';
    if (s) label = base === 'category' ? `目录「${s}」页面搭建` : `子目录「${s}」页面搭建`;
  ...
  const loadBlocks = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchAPI<PageBlock[]>(`/page-blocks?pageType=${encodeURIComponent(pageType)}`);
      setBlocks(data);
    } catch {
      toast.error('加载失败');
    } finally {
  ...
  const handleAddBlock = async () => {
    try {
      const block = await fetchAPI<PageBlock>('/page-blocks', {
        method: 'POST',
        body: JSON.stringify({ pageType, blockType: newBlockType, sortOrder: blocks.length, isVisible: true }),
      });
  ...
  const handleEditSave = async (id: number, data: { config: Record<string, unknown>; title?: string }) => {
    try {
      await fetchAPI(`/page-blocks/${id}`, { method: 'PUT', body: JSON.stringify(data) });
  ...
  const handleToggle = async (id: number) => {
    ...
    if (!block) return;
    try {
      await fetchAPI(`/page-blocks/${id}`, { method: 'PUT', body: JSON.stringify({ isVisible: !block.isVisible }) });
  ...
  // ── Preview width ──
  const previewMaxWidth = { desktop: 'max-w-5xl', tablet: 'max-w-2xl', mobile: 'max-w-sm' }[previewWidth];
    ...
    <div className="-m-6 h-[calc(100vh-56px)] flex flex-col">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border bg-gray-50 p-0.5">
            {([
              ...
              ['tablet', Tablet],
              ['mobile', Smartphone],
            ] as [PreviewWidth, typeof Monitor][]).map(([w, Icon]) => (
              <button
                key={w}
                onClick={() => setPreviewWidth(w)}
                className={`rounded-md px-2.5 py-1.5 transition-colors ${
                  previewWidth === w ? 'bg-white shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600'
                }`}
      ...
      {((basePageType !== 'home' && !slug) || slug) && (
        <div className={`px-4 py-2 text-xs flex-shrink-0 ${
          slug ? 'bg-blue-50 text-blue-700 border-b border-blue-100' : 'bg-amber-50 text-amber-700 border-b border-amber-100'
        }`}>
          {slug
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[340px] min-w-[280px] border-r bg-gray-50 flex flex-col flex-shrink-0">
          <div className="px-4 py-2 border-b bg-white flex items-center justify-between flex-shrink-0">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">区块列表</span>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-100" ref={previewRef}>
          {/* Preview container */}
          <div className={`mx-auto py-8 px-4 transition-all duration-200 ${previewMaxWidth}`}>
            {blocks.length === 0 ? (
              <div className="rounded-xl border border-dashed bg-white py-24 text-center">
                <p className="text-gray-400">点击左上角「添加区块」开始搭建页面</p>
                <p className="mt-1 text-xs text-gray-300">添加后可在右侧实时预览效果</p>
              </div>
            ) : (
      ...
      {/* Add block modal */}
      {addingBlock && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setAddingBlock(false)}>
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold text-lg mb-4">添加新区块</h3>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">选择区块类型</label>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setAddingBlock(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
              <button onClick={handleAddBlock} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white">
                添加
              </button>
            </div>
          </div>
