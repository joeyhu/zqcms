...
export const sitemapRoutes = new Elysia({ prefix: '/api' })
  .get('/sitemap', async (ctx) => {
    ...
    for (const post of posts) urls.push(`<url><loc>${domain}/${post.category.slug}/${post.slug}</loc><lastmod>${post.updatedAt.toISOString()}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>`);
