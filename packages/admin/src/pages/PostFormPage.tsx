...
export function PostFormPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const body = {
      ...
      isFeatured, coverImage: coverImage || null,
    };

    try {
      if (isEdit) {
        await fetchAPI(`/posts/by-id/${id}`, { method: 'PUT', body: JSON.stringify(body) });
        toast.success('文章已更新');
      } else {
        await fetchAPI('/posts', { method: 'POST', body: JSON.stringify(body) });
        toast.success('文章已创建');
      }
      navigate('/posts');
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate('/posts')} className="text-sm text-gray-500 hover:text-gray-700">
          返回列表
        </button>
      </div>
